...
from filestore import FileData, Store, MemoryStorage, LocalStorage, FileField, FileStore, LocalEngine
...
local_store = LocalStorage(fields=fields, config={"destination": file_destination, "filters": size_filter})
mem_store = MemoryStorage(name="book", count=2)
s3_store = S3Storage(fields=fields, config={"destination": s3_file_destination, "filters": size_filter})
fields_2 = [
...
multi_store = FileStore(fields=fields_2, config={"destination": file_destination,
                                                 "StorageEngine": LocalEngine, "filters": size_filter})
app = FastAPI()


@app.post("/local_store")
